...
from utils.dataloader_us import usDataloader
...
def train(epoch, trainset):
    ...
    for image, label in tqdm(
        trainset, ncols=60, desc="train", unit="b", leave=None
    ):
...
def eval(epoch, testset, datatype="val"):
    ...
    net.eval()
    for image, label in tqdm(
        testset, ncols=60, desc=datatype, unit="b", leave=None
    ):
        image, label = image.to(device), label.to(device)
        with autocast(enabled=True):
            pred = net.forward(image)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-annotation", type=str, default="Data_Entry_2017_jpg.csv"
    )
    ...
    if cfg.train_type == "lora":
        lora_model = LoRA_ViT_timm(
            model, r=cfg.rank, alpha=cfg.alpha, num_classes=cfg.num_classes
        )
        # lora_model = LoRA_ViT(
        #     model, r=cfg.rank, alpha=cfg.alpha, num_classes=cfg.num_classes
        # )
        ...
        num_params = sum(
            p.numel() for p in adapter_model.parameters() if p.requires_grad
        )
        logging.info(f"trainable parameters: {num_params/2**20:.4f}M")
    ...
    trainset, valset, testset = usDataloader(cfg)
